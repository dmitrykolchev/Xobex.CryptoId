...
public class CryptoIdEncoderTests : CryptoIdTestBase
{
    ...
    {
        // Arrange & Act
        var encoder = CryptoIdFactory.Create<long>(IdCiperAlgorithm.AesGcm, TestKey, TestSalt);
    ...
    {
        // Arrange & Act
        var encoder = CryptoIdFactory.Create<long>(IdCiperAlgorithm.Speck64_128, TestKey, TestSalt);
    ...
    {
        // Arrange & Act
        var encoder = CryptoIdFactory.Create<int>(IdCiperAlgorithm.Speck32_64, TestKey, TestSalt);
    ...
    public void Create_WithSpeck32_64AndLongType_ThrowsArgumentException()
    {
        ...
        var ex = ThrowsException<ArgumentException>(() =>
            CryptoIdFactory.Create<long>(IdCiperAlgorithm.Speck32_64, TestKey, TestSalt)
        );
    ...
    public void Create_WithSpeck64_128AndIntType_ThrowsArgumentException()
    {
        // Arrange & Act & Assert
        var ex = ThrowsException<ArgumentException>(() =>
            CryptoIdFactory.Create<int>(IdCiperAlgorithm.Speck64_128, TestKey, TestSalt)
        );
        Assert.IsTrue(ex.Message.Contains("unsupported algorithm", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void Create_WithAesGcmAndIntType_ThrowsArgumentException()
    {
        // Arrange & Act & Assert
        var ex = ThrowsException<ArgumentException>(() =>
            CryptoIdFactory.Create<int>(IdCiperAlgorithm.AesGcm, TestKey, TestSalt)
        );
        Assert.IsTrue(ex.Message.Contains("unsupported algorithm", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void Create_WithUnsupportedType_ThrowsNotSupportedException()
    {
        ...
        var ex = ThrowsException<NotSupportedException>(() =>
            CryptoIdFactory.Create<short>(IdCiperAlgorithm.AesGcm, TestKey, TestSalt)
        );
    ...
    public void Create_WithNullSalt_UsesDefaultSalt()
    {
        ...
        var encoder = CryptoIdFactory.Create<long>(IdCiperAlgorithm.AesGcm, TestKey, null);
    ...
    public void Create_WithNullKey_ThrowsArgumentException()
    {
        // Arrange & Act & Assert
        var ex = ThrowsException<ArgumentException>(() =>
            CryptoIdFactory.Create<long>(IdCiperAlgorithm.AesGcm, null!, TestSalt)
        );
    ...
    public void Create_WithEmptyKey_ThrowsArgumentException()
    {
        // Arrange & Act & Assert
        var ex = ThrowsException<ArgumentException>(() =>
            CryptoIdFactory.Create<long>(IdCiperAlgorithm.AesGcm, "", TestSalt)
        );
        Assert.IsTrue(ex.Message.Contains("null", StringComparison.OrdinalIgnoreCase) ||
